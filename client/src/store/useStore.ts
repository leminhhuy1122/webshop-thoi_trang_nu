...
const normalizeProduct = (p: any): any => {
  if (p && p.category && typeof p.category === 'object') {
    return {
      ...p,
...
export const useStore = create<AppState>((set, get) => ({
  ...
  initSocket: () => {
    ...
    socket.on('products_updated', (products) => {
      set({ products: normalizeProducts(products) });
    });
  ...
  fetchInitialData: async () => {
    ...
    try {
      ...
      if (token) {
        ...
        fetchPromises.push(fetch('/api/orders', { headers }));
      }

      const results = await Promise.all(fetchPromises);
      const productsRes = results[0];
      const categoriesRes = results[1];
      const ordersRes = token ? results[2] : null;
  ...
  createProduct: async (productData) => {
    try {
      ...
      if (res.ok) {
        ...
        const catExists = currentCategories.find(c => c.name.toLowerCase() === normalizedProduct.category.toLowerCase());
        let updatedCategories = currentCategories;
        if (normalizedProduct.category && !catExists) {
          const newCat = { id: `cat-${Date.now()}`, name: normalizedProduct.category, createdAt: new Date().toISOString() };
