...
export function Account() {
  ...
  const { user, login, logout, wishlistItems, addToast, fetchInitialData } = useStore();
  ...
  const handleRedeemPoints = async (pointsRequired: number) => {
    ...
    try {
      const res = await fetch('/api/auth/redeem-points', {
        method: 'POST',
        headers: {
      ...
      if (res.ok) {
        // Cập nhật điểm cho user trong store
        login({
          ...user,
          points: data.user.points
        });
        addToast(`Đổi thành công mã giảm giá ${data.voucherCode} bằng ${pointsRequired.toLocaleString('vi-VN')} điểm!`, 'success');
        // Tải lại các mã giảm giá mới từ database
        fetchInitialData();
      } else {
        addToast(data.message || 'Đổi điểm thất bại', 'error');
      }
    } catch (err) {
          ...
          {activeTab === 'membership' && (
            <div>
              ...
              {(() => {
                ...
                let tierName = 'Thành Viên Đồng';
                let nextTierName = 'Bạc';
                let nextTierThreshold = 5000000;
                let progressPercentage = Math.min(100, (totalSpent / 5000000) * 100);

                if (totalSpent >= 15000000) {
                  tierName = 'Thành Viên Vàng';
                  nextTierName = '';
                  nextTierThreshold = 15000000;
                  progressPercentage = 100;
                } else if (totalSpent >= 5000000) {
                  tierName = 'Thành Viên Bạc';
                  nextTierName = 'Vàng';
                  nextTierThreshold = 15000000;
                  progressPercentage = Math.min(100, ((totalSpent - 5000000) / (15000000 - 5000000)) * 100);
                }
                  ...
                  <>
                    <div className="bg-gradient-to-br from-[#E8CFCF] to-[#d8b8b8] rounded-3xl p-8 mb-8 text-black shadow-inner">
                      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                        <div>
                          ...
                          <h2 className="text-4xl font-serif italic mb-4">{tierName}</h2>
                          {nextTierName ? (
                            <p className="text-sm opacity-80 max-w-sm">
                              Bạn cần chi tiêu thêm <strong className="font-bold">{new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(remainingToNext)}</strong> để thăng hạng {nextTierName} và nhận thêm nhiều ưu đãi.
                            </p>
                          ) : (
                            <p className="text-sm opacity-80 max-w-sm">
                              Chúc mừng! Bạn đã đạt hạng thành viên cao nhất và nhận mọi đặc quyền tối đa của AURA.
                            </p>
                          )}
                        </div>
                        <div className="w-full md:w-64">
                          <div className="flex justify-between text-xs font-bold mb-2">
                            <span>{totalSpent >= 15000000 ? 'Vàng' : totalSpent >= 5000000 ? 'Bạc' : 'Đồng'}</span>
                            <span>{nextTierName}</span>
                          </div>
                          <div className="w-full bg-white/40 h-2 rounded-full overflow-hidden mb-2">
                            <div className="bg-black h-full rounded-full" style={{ width: `${progressPercentage}%` }}></div>
                          </div>
                          <div className="text-[10px] text-right opacity-80 uppercase tracking-widest">
                            {new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(totalSpent)} / {new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(nextTierThreshold)}
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="mb-10">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div className={`border rounded-2xl p-6 ${totalSpent < 5000000 ? 'border-black bg-white relative shadow-lg shadow-black/5' : 'border-black/10 opacity-50 bg-gray-50'}`}>
                          {totalSpent < 5000000 && <div className="absolute -top-3 left-6 bg-black text-white text-[10px] font-bold uppercase tracking-widest px-3 py-1 rounded-full animate-pulse">Hiện tại</div>}
                          <h4 className="text-lg font-serif italic mb-2 mt-2">Đồng</h4>
                        ...
                        <div className={`border rounded-2xl p-6 ${totalSpent >= 5000000 && totalSpent < 15000000 ? 'border-black bg-white relative shadow-lg shadow-black/5' : 'border-black/10 opacity-50 bg-gray-50'}`}>
                          {totalSpent >= 5000000 && totalSpent < 15000000 && <div className="absolute -top-3 left-6 bg-black text-white text-[10px] font-bold uppercase tracking-widest px-3 py-1 rounded-full animate-pulse">Hiện tại</div>}
                        ...
                        <div className={`border rounded-2xl p-6 ${totalSpent >= 15000000 ? 'border-[#D4AF37] bg-white relative shadow-lg shadow-black/5' : 'border-black/10 opacity-50 bg-[#FAF9F6]'}`}>
                          {totalSpent >= 15000000 && <div className="absolute -top-3 left-6 bg-black text-white text-[10px] font-bold uppercase tracking-widest px-3 py-1 rounded-full animate-pulse">Hiện tại</div>}
                          <h4 className="text-lg font-serif italic mb-2 mt-2 text-[#D4AF37]">Vàng</h4>
                    ...
                    <div>
                      <div className="flex justify-between items-end mb-6">
                        ...
                        <div className="text-right">
                          ...
                          <span className="text-2xl font-serif italic font-bold">{(user.points || 0).toLocaleString('vi-VN')}</span>
                        </div>
                      </div>
                          ...
                          <div key={i} className={`border rounded-2xl p-5 flex flex-col justify-between gap-4 shadow-sm ${reward.color}`}>
                            ...
                            <button 
                              ...
                              className={`w-full py-2.5 rounded-full text-[10px] font-bold uppercase tracking-widest transition-colors flex items-center justify-center gap-2 ${
                                (user.points || 0) >= reward.pts 
                                  ? 'bg-black text-white hover:bg-neutral-800' 
                                  : 'bg-black/5 text-black/40 cursor-not-allowed'
                              }`}
                              disabled={(user.points || 0) < reward.pts || isUpdating}
                            >
                              {isUpdating && (user.points || 0) >= reward.pts ? <span className="w-3 h-3 border border-white/20 border-t-white rounded-full animate-spin"></span> : null}
                              {(user.points || 0) >= reward.pts ? 'Đổi ngay' : 'Chưa đủ điểm'}
                            </button>
                          </div>
                        ))}
                      </div>
